...
if (isset($_POST['submit']) && (Sesion::existe('user') && Sesion::leer('user') === 'admin')) {
        ...
        <tbody id="tbody">
            ...
            if (Sesion::existe('user') && Sesion::leer('user')->getRol() === 'admin') {
                ...
                for ($i = 0; $i < count($bandas); $i++) {
                    ...
                    $optBandas .= '<option value="' . $i . '">' . $bandas[$i]->getNombre() . '</option>';
                }
                for ($i = 0; $i < count($modos); $i++) {
                    # Rellenamos el SELECT
                    $optModos .= '<option value="' . $i . '">' . $modos[$i]->getNombre() . '</option>';
                }
